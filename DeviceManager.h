...
struct DeviceInfo {
    ...
    DeviceInfo() : type(DeviceType::UNKNOWN), status(DeviceStatus::DISCONNECTED), port(0), bidirectional(true) {}
    
    DeviceInfo(const std::string& deviceId, const std::string& deviceName, DeviceType deviceType)
        : id(deviceId), name(deviceName), type(deviceType), status(DeviceStatus::DISCONNECTED), port(0), bidirectional(true) {}
};
...
public:
    ...
    // Status
    virtual DeviceStatus getDeviceStatus(const std::string& deviceId) = 0;
...
public:
    ...
    // Callbacks
    void setDeviceEventCallback(std::function<void(const std::string&, DeviceStatus)> callback);
    ...
    std::map<std::string, DeviceStatus> getAllDeviceStatuses() const;
...
private:
    ...
    // Callbacks
    std::function<void(const std::string&, DeviceStatus)> deviceEventCallback_;
