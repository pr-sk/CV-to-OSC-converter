...
bool DeviceManager::connectDevice(const std::string& deviceId) {
    ...
    device.status = DeviceConnectionState::CONNECTING;
    updateDeviceStatus(deviceId, DeviceConnectionState::CONNECTING);
    
    if (handler->connect(device)) {
        device.status = DeviceConnectionState::CONNECTED;
        connectedDevices_[deviceId] = device;
        updateDeviceStatus(deviceId, DeviceConnectionState::CONNECTED);
        
        std::cout << "Device connected: " << device.name << " (" << deviceId << ")" << std::endl;
        return true;
    } else {
        device.status = DeviceConnectionState::ERROR;
        updateDeviceStatus(deviceId, DeviceConnectionState::ERROR);
...
bool DeviceManager::disconnectDevice(const std::string& deviceId) {
    ...
    device.status = DeviceConnectionState::DISCONNECTED;
    connectedDevices_.erase(connectedIt);
    updateDeviceStatus(deviceId, DeviceConnectionState::DISCONNECTED);
...
void DeviceManager::setDeviceEventCallback(std::function<void(const std::string&, DeviceConnectionState)> callback) {
...
DeviceConnectionState DeviceManager::getDeviceStatus(const std::string& deviceId) const {
    std::lock_guard<std::mutex> lock(devicesMutex_);
        ...
        try {
            ...
            if (autoReconnectEnabled_) {
                ...
                for (auto& [deviceId, device] : devices_) {
                    if (device.status == DeviceConnectionState::ERROR || device.status == DeviceConnectionState::DISCONNECTED) {
                        auto* handler = getHandlerForType(device.type);
...
void DeviceManager::updateDeviceStatus(const std::string& deviceId, DeviceConnectionState status) {
