    ...
    try {
        ...
        if (j.contains("profiles") && j["profiles"].is_object()) {
            ...
            for (auto it = j["profiles"].begin(); it != j["profiles"].end(); ++it) {
                ...
                profile.audioOutputDevice = profileJson.value("audio_output_device", "");
                profile.autoStartAudio = profileJson.value("auto_start_audio", true);
                profile.rememberDevices = profileJson.value("remember_devices", true);
...
bool Config::saveToFile(const std::string& filename) {
    try {
        ...
        for (const auto& [name, profile] : profiles) {
            ...
            profileJson["audio_output_device"] = profile.audioOutputDevice;
            profileJson["auto_start_audio"] = profile.autoStartAudio;
            profileJson["remember_devices"] = profile.rememberDevices;
...
void Config::setAudioOutputDevice(const std::string& device) {
    getActiveProfile().audioOutputDevice = device;
}

void Config::setAutoStartAudio(bool autoStart) {
    getActiveProfile().autoStartAudio = autoStart;
}

void Config::setRememberDevices(bool remember) {
    getActiveProfile().rememberDevices = remember;
}
