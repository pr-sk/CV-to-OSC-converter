...
void PerformanceMonitor::enableOSCWarningSuppression(bool enable, std::chrono::seconds duration) {
    config.suppressOSCWarnings = enable;
    config.oscWarningSuppressDuration = duration;
    
    if (!enable) {
        oscWarningsCurrentlySuppressed = false;
    }
    
    ErrorHandler::getInstance().logInfo(
        enable ? "OSC warning suppression enabled" : "OSC warning suppression disabled",
        enable ? "Suppression duration: " + std::to_string(duration.count()) + " seconds" : ""
    );
}
...
void PerformanceMonitor::recordOSCMessageFailed() {
    ...
    // Check if OSC warning suppression is enabled
    if (config.suppressOSCWarnings) {
        auto now = std::chrono::steady_clock::now();
        
        // Check if we're currently in suppression period
        if (!oscWarningsCurrentlySuppressed) {
            // Log the first warning and start suppression period
            ErrorHandler::getInstance().logWarning(
                "OSC message failed", 
                "Total failed messages: " + std::to_string(oscFailedCounter.load()) + 
                ". Further warnings will be suppressed for " + 
                std::to_string(config.oscWarningSuppressDuration.count()) + " seconds."
            );
            
            lastOSCWarningTime = now;
            oscWarningsCurrentlySuppressed = true;
        } else {
            // Check if suppression period has expired
            auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(now - lastOSCWarningTime);
            if (elapsed >= config.oscWarningSuppressDuration) {
                // Log accumulated failures and reset suppression
                ErrorHandler::getInstance().logWarning(
                    "OSC message failures during suppression period", 
                    "Total failed messages: " + std::to_string(oscFailedCounter.load())
                );
                
                lastOSCWarningTime = now;
                // Keep suppression active for next period
            }
            // Otherwise, suppress the warning (do nothing)
        }
    } else {
        // No suppression - log every failure
        ErrorHandler::getInstance().logWarning(
            "OSC message failed", 
            "Failed message count: " + std::to_string(oscFailedCounter.load())
        );
    }
}
