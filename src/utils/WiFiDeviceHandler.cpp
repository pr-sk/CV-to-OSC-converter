...
bool WiFiDeviceHandler::isDeviceAvailable(const std::string& deviceId) {
    ...
    return it != wifiDevices_.end() && it->second.status != DeviceConnectionState::ERROR;
}
...
bool WiFiDeviceHandler::connect(const DeviceInfo& device) {
    ...
    wifiDevice.status = DeviceConnectionState::CONNECTED;
...
DeviceConnectionState WiFiDeviceHandler::getDeviceStatus(const std::string& deviceId) {
    std::lock_guard<std::mutex> lock(wifiMutex_);
